...
import static com.cursee.moandjiezana.toml.ValueWriters.WRITERS;
...
public class TomlWriter {
  ...
  private TomlWriter(int keyIndentation, int tableIndentation, int arrayDelimiterPadding,
      TimeZone timeZone, boolean showFractionalSeconds) {
    this.indentationPolicy = new IndentationPolicy(keyIndentation, tableIndentation,
   ...
   *
   * @param from   the object to be written
   * @param target the OutputStream to which the TOML will be written. The stream is NOT closed
  ...
  /**
   * Write an Object in TOML to a {@link Writer}. You MUST ensure that the {@link Writer}s's
   * encoding is set to UTF-8 for the TOML to be valid.
   *
  ...
  public void write(Object from, Writer target) throws IOException {
    ...
    if (valueWriter == MAP_VALUE_WRITER || valueWriter == OBJECT_VALUE_WRITER) {
      ...
      throw new IllegalArgumentException("An object of class " + from.getClass().getSimpleName()
          + " cannot produce valid TOML. Please pass in a Map or a custom type.");
    }
  }
  ...
  public static class Builder {
    ...
    /**
     * @param spaces number of spaces to put between opening square bracket and first item and
     *               between closing square bracket and last item
