...
import static com.cursee.moandjiezana.toml.IdentifierConverter.IDENTIFIER_CONVERTER;
  ...
  static Results run(String tomlString) {
    ...
    for (int i = index.get(); i < tomlString.length(); i = index.incrementAndGet()) {
      ...
      if (c == '#' && !inComment) {
        ...
        Identifier id = IDENTIFIER_CONVERTER.convert(tomlString, index,
            new Context(null, line, results.errors));
      ...
      } else if (!inComment && identifier != null && identifier.isKey() && value == null
          && !Character.isWhitespace(c)) {
        value = ValueReaders.VALUE_READERS.convert(tomlString, index,
            new Context(identifier, line, results.errors));
