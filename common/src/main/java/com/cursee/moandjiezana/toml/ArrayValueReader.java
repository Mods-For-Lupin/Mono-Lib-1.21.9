...
import static com.cursee.moandjiezana.toml.ValueReaders.VALUE_READERS;
...
class ArrayValueReader implements ValueReader {
  ...
  public Object read(String s, AtomicInteger index, Context context) {
    ...
    if (!terminated) {
      errors.unterminated(context.identifier.getName(), s.substring(startIndex),
          startLine);
    }
  ...
  private boolean isHomogenousArray(Object o, List<?> values) {
    return values.isEmpty() || values.get(0).getClass().isAssignableFrom(o.getClass())
        || o.getClass().isAssignableFrom(values.get(0).getClass());
  }
}
