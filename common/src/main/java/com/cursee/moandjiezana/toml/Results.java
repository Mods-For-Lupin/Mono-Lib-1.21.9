...
class Results {
  ...
  void startTableArray(Identifier identifier, AtomicInteger line) {
    ...
    for (int i = 0; i < tableParts.length; i++) {
      ...
      if (currentContainer.get(tablePart) instanceof Container.TableArray currentTableArray) {
      ...
      } else if (currentContainer.get(tablePart) instanceof Container.Table nextTable
          && i < tableParts.length - 1) {
        stack.push(nextTable);
      } else if (currentContainer.accepts(tablePart)) {
        Container newContainer =
            i == tableParts.length - 1 ? new Container.TableArray() : new Container.Table();
  ...
  void startTables(Identifier id, AtomicInteger line) {
    ...
    for (int i = 0; i < tableParts.length; i++) {
      String tablePart = tableParts[i].name;
      Container currentContainer = stack.peek();
      if (currentContainer.get(tablePart) instanceof Container nextTable) {
